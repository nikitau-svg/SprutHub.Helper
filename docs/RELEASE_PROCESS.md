...
Постоянные ветки `main` и `beta` уже созданы. Не создавайте новую `beta` от устаревшей `main` и не перемещайте опубликованные теги.
...
Обновите `versionName`, `versionCode` и раздел версии в `CHANGELOG.md`, выполните автоматические проверки, затем создайте аннотированный тег `vX.Y.Z-beta.N` на том же коммите и отправьте ветку и тег. Workflow **Publish beta release** заново проверит точный tagged commit, соберёт и проверит подписанный APK, создаст SHA-256, опубликует GitHub prerelease и запустит Telegram-уведомление. Ручная загрузка файлов и локальный `gh` для beta-релиза не нужны. Теги с дефисом исключены из стабильного workflow и никогда не создают stable draft.

Аппаратный E2E обязателен перед переносом в `main`. Если владелец явно переносит E2E для промежуточной beta (например, телефон временно недоступен), это отмечается в CHANGELOG/release notes; такая сборка остаётся prerelease и не считается кандидатом в стабильную ветку.
...
Старый Release и тег `v0.1.0` уже удалены. Это не очистило раннюю Git-историю; отдельное решение о её переписывании зафиксировано в [аудите репозитория](REPOSITORY_AUDIT.md).
...
Для условной проверенной версии `0.6.0` команды выглядели бы так (выполнять только после фактического слияния beta в `main` и смены `versionName` с beta на стабильный):

```bash
git switch main
git pull --ff-only origin main
git tag -a v0.6.0 -m "SprutHub Helper 0.6.0"
