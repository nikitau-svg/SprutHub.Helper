...
Каждый push в `beta` запускает lint, unit-тесты и `assembleDebug`, затем сохраняет `app-debug.apk` как GitHub Actions artifact на 14 дней. В workflow нет пользовательских секретов и signing-keystore.
